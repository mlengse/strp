...
module.exports = {
  ...
  add: async (values) => {
    ...
    return Counter.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    ...
    const entry = await Counter.update(params, data, { multi: true });

    // Update relational data and return the entry.
    return Counter.updateRelations(Object.assign(params, { values: relations }));
  },
  ...
  remove: async params => {
    ...
    await Promise.all(
      Counter.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Counter.attributes).reduce((acc, curr) => {
      switch (Counter.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
